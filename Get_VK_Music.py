...
from tkinter import *
from tkinter.ttk import (Button, Entry, Label, Scrollbar,
...
class Main:
    ...
    def __init__(self, root):
        # запускаемся, пытаемся автоматически авторизироваться
        Gui(root).try_engage()
...
class Logic():
    ...
    def auth_log(self, log, pswrd):
        ...
        global api
        session = vk.AuthSession(app_id=self.app_id_,
                                 user_login=log,
                                 user_password=pswrd,
                                 scope="audio, offline")
    ...
    def start_download(self):
        ...
        self.progress_bar = Progressbar(self.frame, orient=HORIZONTAL,
        ...
        try:
            ...
            for i in music:
                ...
                self.progress_bar.step()        # увеличиваем шаг прогресс бара
                root.update()
...
class Gui(Logic):
    ...
    def __init__(self, root):
        ...
        self.status = Frame(root, heigh="50",
                            style="My.TFrame").pack(side='bottom', fill='x')

        self.label = Label(self.panel_frame,
                           text="Вы не авторизированы", style="Panel.TLabel")

        self.label.place(x=600, y=13)

        self.label_frame = Label(self.frame,
                                 text="Сколько последних\nзаписей показать?",
                                 style="Frame.TLabel").place(x=20, y=70)

        self.but1 = Button(self.panel_frame,
                           text="Вход",
                           command=self.login_window).place(x=25, y=13)

        self.but3 = Button(self.frame, text="Список",
                           command=self.music_but, state="disabled")
        self.but3.place(x=25, y=150)

        self.but4 = Button(self.frame, text="Скачать",
                           state="disabled", command=self.start_download)
        self.but4.place(x=25, y=300)

        self.text = Text(self.frame, height=18,
                         width=75, font='Calibri 11',
                         wrap=WORD, state="disabled")
        self.text.place(x=250, y=90)

        self.text_name = Label(self.frame,
                               text="Список музыки",
        ...
        # клик ПКМ по верхнему фрейму меняет стиль оформления
        self.panel_frame.bind('<3>', self.change_theme)
    ...
    def login_window(self):
        ...
        self.but_enter = Button(self.lwin, text="Вход",
                                command=self.log_pass).place(x=210, y=150)
    ...
    def use_theme_2(self):
        ...
        self.style.configure("TButton", background=yellow,
                             padding=2, width=10, relief="ridge")

        self.style.configure("Horizontal.TProgressbar",
                             background=bg_color_main)  # прогресс бар
        ...
        # лэйблы
        self.style.configure("Panel.TLabel", background=bg_color_main,
                             foreground="black", font='Calibri 12')

        self.style.configure("Frame.TLabel", background=bg_color_frame,
                             foreground="white", font='Calibri 12')
    ...
    def use_theme_1(self):
        ...
        # кнопки
        self.style.configure("TButton", background="#94b9ff",
                             padding=2, width=10, relief="ridge")
